...
(function () {
    ...
    function initPredictionsPage() {
        ...
        seriesSearch.addEventListener('input', debounce(async function () {
            ...
            try {
                ...
                dropdown.innerHTML = results.map(r =>
                    `<div class="autocomplete-item" data-series="${escapeHtml(r.series_name)}">
                        <span>${escapeHtml(r.series_name)}</span>
                ...
                dropdown.innerHTML = `<div class="autocomplete-item" style="color:var(--danger)">${escapeHtml(e.message)}</div>`;
                show(dropdown);
            }
        ...
        document.addEventListener('click', function (e) {
            if (!e.target.closest('.search-wrapper')) hide(dropdown);
        });
        ...
        async function selectSeries(name) {
            ...
            try {
                ...
                document.getElementById('seriesFormulaCount').textContent = `${data.total} formulas, ${data.withLab} with Lab data`;
                renderAnchors(formulasData);
            } catch (e) {
                anchorsBody.innerHTML = `<tr><td colspan="6" class="text-danger">${escapeHtml(e.message)}</td></tr>`;
            }
        ...
        function renderAnchors(formulas) {
            ...
            const filtered = formulas.filter(f =>
                !filter || f.pmsNumber.toLowerCase().includes(filter) ||
                f.colorPart.toLowerCase().includes(filter) ||
                f.itemCode.toLowerCase().includes(filter)
            );

            document.getElementById('totalAnchors').textContent = filtered.filter(f => f.hasLabData).length;

            anchorsBody.innerHTML = filtered.map(f => {
                const disabled = !f.hasLabData;
                return `<tr class="${disabled ? 'disabled-row' : ''}">
                    <td class="checkbox-cell">
                        <input type="checkbox" class="anchor-cb" value="${escapeHtml(f.itemCode)}"
                               ${disabled ? 'disabled title="No Lab reference data available"' : ''}>
                    </td>
                    <td>${f.hex ? `<span class="color-swatch" style="background:${f.hex}"></span>` : '—'}</td>
                    <td><strong>${escapeHtml(f.pmsNumber)}</strong></td>
                    <td>${escapeHtml(f.itemCode)}</td>
                    <td style="font-size:0.8rem">${escapeHtml(f.topComponents)}</td>
                    <td>${f.hasLabData ? '<span class="badge badge-success">OK</span>' : '<span class="badge badge-muted">N/A</span>'}</td>
                </tr>`;
            }).join('');
        ...
        function updateSelectionCount() {
            const checked = anchorsBody.querySelectorAll('.anchor-cb:checked').length;
            document.getElementById('selectedCount').textContent = checked;
            generateBtn.disabled = checked === 0;

            const warning = document.getElementById('anchorWarning');
            if (checked > 0 && checked < 20) {
                document.getElementById('anchorWarningText').textContent =
                    `Only ${checked} anchors selected. Predictions are more reliable with 20+ anchors.`;
                show(warning);
            } else {
                hide(warning);
            }
        ...
        anchorsBody.addEventListener('change', updateSelectionCount);

        document.getElementById('selectAll')?.addEventListener('click', function () {
            anchorsBody.querySelectorAll('.anchor-cb:not(:disabled)').forEach(cb => cb.checked = true);
            updateSelectionCount();
        });
        ...
        document.getElementById('headerCheckbox')?.addEventListener('change', function () {
            anchorsBody.querySelectorAll('.anchor-cb:not(:disabled)').forEach(cb => cb.checked = this.checked);
            updateSelectionCount();
        });

        // Generate predictions
        generateBtn?.addEventListener('click', async function () {
            const codes = [...anchorsBody.querySelectorAll('.anchor-cb:checked')].map(cb => cb.value);
            ...
            try {
                ...
                document.getElementById('resultsSummary').innerHTML =
                    `<span><strong>${data.total}</strong> predictions generated from <strong>${data.anchorCount}</strong> anchors in <strong>${data.elapsed_ms}ms</strong></span>` +
                    (data.skipped > 0 ? `<span class="text-muted">${data.skipped} colors skipped (no components)</span>` : '');

                const warnDiv = document.getElementById('resultsWarnings');
                warnDiv.innerHTML = (data.warnings || []).map(w =>
                    `<div class="warning-banner"><span class="warning-banner-icon">&#9888;</span> ${escapeHtml(w)}</div>`
                ).join('');

                renderResults(predictionsData);
            } catch (e) {
                document.getElementById('resultsSummary').innerHTML = `<span class="text-danger">${escapeHtml(e.message)}</span>`;
            }
        ...
        function renderResults(predictions) {
            ...
            const filtered = predictions.filter(p =>
                !filter || p.pmsNumber.toLowerCase().includes(filter) ||
                p.pmsName.toLowerCase().includes(filter)
            );

            const resultsBody = document.getElementById('resultsBody');
            resultsBody.innerHTML = filtered.map((p, idx) => {
                ...
                return `<tr data-idx="${idx}">
                    ...
                    <td><strong>${escapeHtml(p.pmsNumber)}</strong></td>
                    <td>${escapeHtml(p.pmsName)}</td>
                    <td>
                    ...
                    <td style="font-size:0.8rem">${escapeHtml(compSummary)}</td>
                        ...
                        <div class="detail-content">
                            <table class="component-table">
                                ...
                                ${p.components.map((c, ci) =>
                                    `<tr><td>${ci + 1}</td><td><strong>${escapeHtml(c.code)}</strong></td><td>${escapeHtml(c.description)}</td><td class="text-right">${(c.percentage * 100).toFixed(2)}%</td></tr>`
                                ).join('')}
                                </tbody>
                            ...
                            ${p.warnings.length > 0 ? `<div class="mt-1" style="font-size:0.8rem;color:var(--warning)">${p.warnings.join('; ')}</div>` : ''}
                        </div>
                    </td>
                </tr>`;
            }).join('');
        ...
        document.addEventListener('click', function (e) {
            ...
            if (delBtn) {
                ...
                if (confirm('Delete this prediction?')) {
                    api('/api/predictions/delete', { method: 'POST', body: { id: pid } })
        ...
        // Result checkboxes
        document.getElementById('resultSelectAll')?.addEventListener('click', function () {
            document.querySelectorAll('.result-cb').forEach(cb => cb.checked = true);
        });
        document.getElementById('resultDeselectAll')?.addEventListener('click', function () {
        ...
        document.getElementById('saveSelectedBtn')?.addEventListener('click', function () {
            const indices = [...document.querySelectorAll('.result-cb:checked')].map(cb => parseInt(cb.dataset.idx));
            const selected = indices.map(i => predictionsData[i]).filter(Boolean);
            if (selected.length === 0) { alert('No predictions selected.'); return; }
            saveBatch(selected);
        });
        ...
        document.getElementById('exportCsvBtn')?.addEventListener('click', function () {
            if (predictionsData.length === 0) return;
            exportCsv(predictionsData);
        });

        function exportCsv(predictions) {
            const header = ['PMS Number', 'Name', 'L*', 'a*', 'b*', 'Confidence', 'Components'];
    ...
    function initCustomLabPage() {
        ...
        // Series autocomplete (same pattern)
        seriesInput.addEventListener('input', debounce(async function () {
            ...
            try {
                ...
                seriesDropdown.innerHTML = results.map(r =>
                    `<div class="autocomplete-item" data-series="${escapeHtml(r.series_name)}">
                        <span>${escapeHtml(r.series_name)}</span>
                        <span class="count">${r.formula_count}</span>
                    </div>`
                ).join('') || '<div class="autocomplete-item" style="color:var(--text-muted)">No series found</div>';
                show(seriesDropdown);
            } catch (e) {
                seriesDropdown.innerHTML = `<div class="autocomplete-item" style="color:var(--danger)">${escapeHtml(e.message)}</div>`;
                show(seriesDropdown);
            }
        ...
        document.addEventListener('click', function (e) {
            if (!e.target.closest('.search-wrapper')) hide(seriesDropdown);
        });
        ...
        matchBtn.addEventListener('click', async function () {
            ...
            try {
                const data = await api('/api/custom-lab/match', {
                    method: 'POST',
                    body: {
                        L: parseFloat(labL.value),
                        a: parseFloat(labA.value),
                        b: parseFloat(labB.value),
                        series: selectedSeries,
                    },
                });
                ...
                document.getElementById('customResultsSummary').innerHTML =
                    `<span>Confidence: <strong>${data.confidence.toFixed(1)}%</strong> | ${data.anchorCount} anchors in series</span>`;
                ...
                document.getElementById('customAnchorsInfo').innerHTML = (data.nearestAnchors || []).map(a =>
                    `<span class="badge badge-info" style="margin-right:0.3rem">${escapeHtml(a.pmsNumber || a.itemCode)} (&#916;E ${a.distance})</span>`
                ).join('');
                ...
                tbody.innerHTML = data.components.map((c, i) => {
                    ...
                    totalPct += pct;
                    return `<tr><td>${i + 1}</td><td><strong>${escapeHtml(c.code)}</strong></td><td>${escapeHtml(c.description)}</td><td class="text-right">${pct.toFixed(2)}%</td></tr>`;
                }).join('');
                document.getElementById('customTotalPct').textContent = totalPct.toFixed(2) + '%';

            } catch (e) {
                document.getElementById('customResultsSummary').innerHTML = `<span class="text-danger">${escapeHtml(e.message)}</span>`;
            }
            hide(document.getElementById('customLoading'));
        });

        // Save custom prediction
        document.getElementById('customSaveBtn')?.addEventListener('click', async function () {
            ...
            try {
                ...
                if (e.message.includes('exists')) {
                    alert('A prediction for this already exists.');
                } else {
                    alert('Save failed: ' + e.message);
                }
            }
    ...
    if (testBtn) {
        testBtn.addEventListener('click', async function () {
            ...
            try {
                ...
                if (data.success) {
                    result.className = 'alert alert-success mb-2';
